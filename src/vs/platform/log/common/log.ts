 ...
 *
 *  【业务逻辑说明 - 日志服务接口】
 *  本文件定义日志服务的核心接口，负责应用的日志记录和管理：
 *
 *  【核心职责】
 *  1. 提供多级别日志记录（Trace, Debug, Info, Warning, Error）
 *  2. 管理日志级别（LogLevel）
 *  3. 支持多个日志记录器（Logger）
 *  4. 处理日志输出（控制台、文件、远程）
 *  5. 提供日志滚动和清理
 ...
 *  【日志级别】
 *  ┌─────────────────────────────────────────────────────────┐
 *  │  LogLevel.Off     - 关闭日志（不记录任何内容）          │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  LogLevel.Trace   - 追踪日志（最详细）                  │
 *  │  用于调试和性能分析                                       │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  LogLevel.Debug   - 调试日志                            │
 *  │  开发时使用，记录详细执行流程                            │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  LogLevel.Info    - 信息日志（默认级别）                │
 *  │  记录重要事件和状态变更                                   │
 *  ├─────────────────────────────────────────────────────────┤
 ...
 *  - ILogService: 日志服务主接口
 *  - ILogger: 日志记录器接口
 *  - ILoggerService: 日志记录器管理服务
 *
 *  【日志输出目标】
 *  - Console: 输出到开发者工具控制台
 *  - File: 输出到日志文件（如 renderer.log）
 *  - Remote: 输出到远程日志服务
 *
 *  【使用场景】
 *  - 记录应用启动和关闭事件
