 ...
 *
 *  【业务逻辑说明 - 对话框服务接口】
 *  本文件定义对话框服务的核心接口，用于显示各类模态对话框：
 *
 *  【核心职责】
 *  1. 显示确认对话框（Confirm Dialog）- 是/否选择
 *  2. 显示输入对话框（Input Dialog）- 文本输入
 *  3. 显示提示对话框（Prompt Dialog）- 多按钮选择
 *  4. 显示文件选择器（File Picker）- 打开/保存文件
 *  5. 支持原生和自定义对话框样式
 *
 *  【对话框类型】
 *  ┌─────────────────────────────────────────────────────────┐
 ...
 *
 *  【文件对话框】
 *  - showOpenDialog: 打开文件/文件夹选择器
 *  - showSaveDialog: 保存文件选择器
 *
 *  【对话框选项】
 *  - type: 类型（Info, Warning, Error, Question）
 *  - title: 对话框标题
 *  - message: 主要消息
 *  - detail: 详细说明
 *  - buttons: 自定义按钮
