 ...
 *
 *  【业务逻辑说明 - 菜单和动作系统】
 *  本文件实现 VSCode/Void 的菜单和动作系统，提供命令的菜单注册和展示逻辑：
 *
 *  【核心职责】
 *  1. 定义菜单项接口（IMenuItem）- 关联命令和菜单
 *  2. 定义子菜单接口（ISubmenuItem）- 嵌套菜单结构
 *  3. 提供 MenuId 类 - 菜单标识符管理
 *  4. 实现 IMenuService 接口 - 菜单服务
 *  5. 支持上下文条件（when）- 条件显示菜单项
 *
 *  【菜单系统架构】
 *  ┌─────────────────────────────────────────────────────────┐
 *  │  MenuId - 菜单标识符                                   │
 ...
 *  │  ├─ Explorer Context Menu（资源管理器右键菜单）       │
 *  │  ├─ Menu Bar（菜单栏）                                 │
 *  │  └─ 其他各种菜单...                                     │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  IMenuItem - 菜单项                                   │
