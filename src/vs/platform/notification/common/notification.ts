 ...
 *
 *  【业务逻辑说明 - 通知服务接口】
 *  本文件定义通知服务的核心接口，负责显示各种通知消息：
 *
 *  【核心职责】
 *  1. 显示提示通知（Info、Warning、Error）
 *  2. 管理通知优先级（Default、Silent、Urgent）
 *  3. 支持通知交互（按钮、进度）
 *  4. 提供通知中心（Notifications Center）
 *  5. 处理免打扰模式（Do Not Disturb）
 *
 *  【通知类型】
 *  ┌─────────────────────────────────────────────────────────┐
 *  │  Severity.Info     - 信息提示（蓝色）                  │
 *  │  示例："文件已保存"                                      │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  Severity.Warning  - 警告提示（黄色）                  │
 *  │  示例："配置格式不正确"                                  │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  Severity.Error    - 错误提示（红色）                   │
 *  │  示例："保存失败：权限不足"                              │
 *  └─────────────────────────────────────────────────────────┘
 *
 *  【通知优先级】
 ...
 *  - info(message): 显示信息通知
 *  - warn(message): 显示警告通知
 *  - error(message): 显示错误通知
 *  - notify(notification): 显示自定义通知
 *  - status(message, options): 显示状态栏消息
 *
 *  【通知属性】
 *  - sticky: 是否常驻（不自动消失）
 *  - silent: 是否静默
