 ...
 *
 *  【业务逻辑说明 - 工作台渲染入口】
 *  本文件是渲染进程的工作台初始化入口，负责：
 *
 *  【核心职责】
 *  1. 创建工作台（Workbench）实例 - 整个 UI 的容器
 *  2. 初始化服务注入容器（ServiceCollection）
 *  3. 设置布局管理器 - 标题栏、侧边栏、编辑器区、面板、状态栏
 *  4. 注册工作台贡献点（Contributions）
 *  5. 处理通知系统（通知中心、提示、状态栏图标）
 *  6. 管理生命周期（启动、恢复、关闭）
 *
 *  【工作台架构】
 *  ┌─────────────────────────────────────────────────────────┐
 ...
 *  【初始化流程】
 *  1. 导入样式（style.js）
 *  2. 创建 ServiceCollection（服务注入容器）
 *  3. 注册单例服务（registerSingletonServiceDescriptors）
 *  4. 创建 Workbench 实例
 *  5. 初始化布局（titlebar, activityBar, sidebar, editor, panel, statusbar）
 *  6. 启动生命周期服务
 *  7. 触发工作台贡献点（LifecyclePhase.Restored）
 *  8. 恢复编辑器状态
 *  9. 显示界面
 *
 *  【关键服务】
 *  - IInstantiationService: 依赖注入容器
 *  - IWorkbenchLayoutService: 布局管理服务
 *  - ILifecycleService: 生命周期管理
 *  - IStorageService: 持久化存储
 *  - IConfigurationService: 配置管理
 *
 *  【进程关系】
 *  Electron Main ──IPC──→ workbench.ts ──ServiceInjection──→ 各功能模块
 *
