 ...
 *
 *  【业务逻辑说明 - Void 代码编辑服务】
 *  本文件实现 Void 的核心代码编辑功能，负责 AI 代码生成和应用的完整流程：
 *
 *  【核心职责】
 *  1. 创建和管理代码编辑区域（DiffArea）
 *  2. 实现 Apply 功能 - 在编辑器中显示 AI 生成的代码修改
 *  3. 实现 Fast Apply 功能 - 流式应用代码修改
 *  4. 管理 Ctrl+K 快速编辑区域（CtrlKZone）
 *  5. 处理用户接受/拒绝代码修改的交互
 *
 *  【核心概念】
 *  ┌─────────────────────────────────────────────────────────┐
 ...
 *  ├─────────────────────────────────────────────────────────┤
 *  │  DiffZone - 差异显示区域                                │
 *  │  - 在编辑器中渲染代码对比（原代码 vs 新代码）             │
 *  │  - 使用 ViewZone 技术嵌入编辑器                         │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  CtrlKZone - Ctrl+K 快速编辑区域                        │
 *  │  - 用户选中的代码范围                                   │
 *  │  - 显示输入框让用户描述想要的修改                       │
 *  └─────────────────────────────────────────────────────────┘
 *
 *  【工作流程】
 *  1. 用户触发 Apply 或 Ctrl+K
 *  2. 构建提示词（包括系统提示、用户提示、上下文）
 *  3. 调用 LLM 生成代码修改
 *  4. 在编辑器中创建 DiffZone 显示修改
 *  5. 用户接受或拒绝修改
 *  6. 应用或撤销代码变更
 *
 *  【核心方法】
 *  - addCtrlKZone(): 添加 Ctrl+K 编辑区域
 *  - startApplying(): 开始应用 AI 生成的代码
 *  - acceptDiff(): 接受代码修改
 *  - rejectDiff(): 拒绝代码修改
 *
 *  【与编辑器的关系】
 *  - 使用 CodeEditorWidget 的 API 创建 ViewZone
 *  - 监听编辑器内容变更事件
 *  - 通过 Decoration 高亮修改区域
 *
