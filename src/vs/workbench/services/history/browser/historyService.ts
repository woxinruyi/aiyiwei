 ...
 *
 *  【业务逻辑说明 - 历史服务实现】
 *  本文件实现编辑器历史管理服务，负责跟踪编辑器历史记录和最近关闭的编辑器：
 *
 *  【核心职责】
 *  1. 管理编辑器历史记录（Editor History）
 *  2. 跟踪最近关闭的编辑器（Recently Closed Editors）
 *  3. 支持导航历史（Go Back/Go Forward）
 *  4. 管理最近打开的文件列表
 *  5. 持久化历史记录到存储
 *
 *  【历史记录类型】
 ...
 *  │  - 记录用户打开过的文件                                   │
 *  │  - 用于快速打开（Quick Open）文件列表                   │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  Navigation History - 导航历史                           │
 *  │  - 记录光标位置变化                                       │
 *  │  - 支持 Go Back/Go Forward 导航                         │
 *  ├─────────────────────────────────────────────────────────┤
 ...
 *  - goBack(): 导航到上一个位置
 *  - goForward(): 导航到下一个位置
 *  - getHistory(): 获取编辑器历史列表
 *  - removeFromHistory(): 从历史中移除
 *  - reopenClosedEditor(): 重新打开最近关闭的编辑器
 *
 *  【使用场景】
 *  - 快速打开文件（Ctrl+P）显示历史文件
 *  - 导航历史（Alt+Left/Right）
 *  - 重新打开关闭的文件
 *  - 清除历史记录
 *
