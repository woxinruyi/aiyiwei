 ...
 *
 *  【业务逻辑说明 - 快捷键编辑服务】
 *  本文件实现快捷键编辑服务，负责管理 keybindings.json 文件的修改：
 *
 *  【核心职责】
 *  1. 添加新的快捷键绑定（addKeybinding）
 *  2. 编辑现有快捷键（editKeybinding）
 *  3. 删除快捷键绑定（removeKeybinding）
 *  4. 重置快捷键到默认值（resetKeybinding）
 *  5. 处理并发编辑请求的队列管理
 *
 *  【快捷键编辑流程】
 *  ┌─────────────────────────────────────────────────────────┐
 *  │  1. 用户通过快捷键编辑器 UI 发起操作                      │
 *  │  2. 服务解析 keybindings.json 文件                       │
 *  │  3. 修改 JSON 内容（添加/编辑/删除）                     │
 *  │  4. 保存文件                                             │
 *  │  5. 快捷键立即生效                                       │
 *  └─────────────────────────────────────────────────────────┘
 *
 ...
 *
 *  【核心接口】
 *  - IKeybindingEditingService: 快捷键编辑服务接口
 *  - ResolvedKeybindingItem: 解析后的快捷键项
 *  - IUserFriendlyKeybinding: 用户友好的快捷键定义
 *
 *  【依赖服务】
 *  - ITextModelService: 解析文本模型
 *  - ITextFileService: 文件保存
 *  - IFileService: 文件操作
 *  - IUserDataProfileService: 用户配置文件
 *
 *  【使用场景】
 *  - 快捷键编辑器 UI
 *  - 命令面板添加快捷键
 ...
 *  - 导入/导出快捷键配置
 *
 *  【与 keybindingsRegistry.ts 的关系】
 *  - keybindingsRegistry.ts 管理运行时快捷键
 *  - keybindingEditing.ts 管理持久化存储
 *
