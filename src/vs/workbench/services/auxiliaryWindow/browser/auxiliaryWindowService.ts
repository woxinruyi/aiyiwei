 ...
 *
 *  【业务逻辑说明 - 辅助窗口服务】
 *  本文件实现辅助窗口服务，支持在主窗口外创建额外的编辑器窗口：
 *
 *  【核心职责】
 *  1. 创建和管理辅助窗口（Auxiliary Window）
 *  2. 支持将编辑器拖拽到独立窗口
 *  3. 同步主窗口的主题、设置和样式
 *  4. 管理辅助窗口的生命周期
 *  5. 支持多种窗口模式（最大化、普通、全屏）
 *
 *  【辅助窗口概念】
 *  ┌─────────────────────────────────────────────────────────┐
 *  │  辅助窗口允许编辑器内容脱离主窗口显示：                 │
 ...
 *
 *  【核心接口】
 *  - IAuxiliaryWindowService: 辅助窗口服务接口
 *  - IAuxiliaryWindow: 辅助窗口实例
 *  - IAuxiliaryWindowOpenOptions: 打开选项
 *
 *  【核心方法】
 *  - open(options): 打开辅助窗口
 *  - onDidOpenAuxiliaryWindow: 窗口打开事件
 *  - createState(): 创建窗口状态
 *  - applyState(): 应用窗口状态
 *
 *  【样式同步】
