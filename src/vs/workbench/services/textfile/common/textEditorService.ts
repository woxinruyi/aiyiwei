 ...
 *
 *  【业务逻辑说明 - 文本编辑器服务】
 *  本文件定义文本编辑器服务的核心接口，负责创建和管理文本编辑器输入：
 *
 *  【核心职责】
 *  1. 创建文本编辑器输入（EditorInput）
 *  2. 根据资源类型选择正确的编辑器输入类型
 *  3. 支持文件、未保存文件、虚拟资源的编辑
 *  4. 与 IEditorResolverService 协同工作
 *
 *  【编辑器输入类型】
 *  ┌─────────────────────────────────────────────────────────┐
 *  │  IFileEditorInput - 文件编辑器输入                    │
 *  │  用于编辑本地文件（file:// 协议）                      │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  UntitledEditorInput - 未保存文件输入                  │
 *  │  用于编辑新建未保存的文本（untitled://）               │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  TextResourceEditorInput - 虚拟资源输入               │
 *  │  用于编辑虚拟资源（如 git 冲突、对比视图）            │
 *  └─────────────────────────────────────────────────────────┘
 *
 *  【核心方法】
 *  - createTextEditor(input): 根据输入类型创建编辑器输入
 *
 *  【与 IEditorResolverService 的关系】
 *  - 本服务的 createTextEditor 方法已标记为 @deprecated
 *  - 推荐使用 IEditorResolverService 替代
 *  - IEditorResolverService 提供更通用的编辑器解析
 *
 *  【使用场景】
 *  - 打开文件时创建编辑器输入
 *  - 新建文件时创建未保存编辑器
