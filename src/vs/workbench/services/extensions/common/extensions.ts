 ...
 *
 *  【业务逻辑说明 - 扩展服务接口】
 *  本文件定义扩展管理的核心接口，是 VSCode/Void 扩展系统的基石：
 *
 *  【核心职责】
 *  1. 管理扩展生命周期（安装、启用、禁用、卸载）
 *  2. 控制扩展主机（Extension Host）进程
 *  3. 处理扩展激活事件（Activation Events）
 *  4. 管理扩展贡献点（Contributions）
 *  5. 提供扩展 API 给扩展使用
 *
 *  【扩展架构】
 *  ┌─────────────────────────────────────────────────────────┐
 *  │              IExtensionService (接口)                  │
 *  │                     ↑ 实现                             │
 *  │           ExtensionService (实现类)                     │
 *  └─────────────────────────────────────────────────────────┘
 *           ↓                              ↓
 *  ┌─────────────────────┐    ┌─────────────────────┐
 *  │ Extension Host 进程  │    │ 扩展市场服务         │
 *  │ (独立 Node.js 进程)  │    │ (查询、下载、安装)   │
 *  └─────────────────────┘    └─────────────────────┘
 *           ↓
 *  ┌─────────────────────┐
 *  │ 激活的扩展实例       │
