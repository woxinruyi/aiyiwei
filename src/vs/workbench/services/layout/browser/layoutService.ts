 ...
 *
 *  【业务逻辑说明 - 工作台布局服务】
 *  本文件定义工作台布局服务的核心接口，管理工作台的 UI 布局：
 *
 *  【核心职责】
 *  1. 管理工作台各部分（Parts）的布局和显示
 *  2. 处理窗口大小变更和响应式布局
 *  3. 支持禅模式（Zen Mode）
 *  4. 管理侧边栏、面板、编辑器的尺寸
 *  5. 处理辅助窗口（Auxiliary Window）布局
 *
 *  【工作台布局结构】
 *  ┌─────────────────────────────────────────────────────────┐
 *  │  TITLEBAR_PART    - 标题栏（可选）                      │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  BANNER_PART      - 通知横幅（可选）                    │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  ┌─────────┬──────────────────────────────┬─────────┐  │
 ...
 *  │  PANEL_PART       - 面板（可选，底部）                    │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  STATUSBAR_PART   - 状态栏（可选）                      │
 *  └─────────────────────────────────────────────────────────┘
 *
 *  【核心方法】
 *  - togglePart(part): 显示/隐藏指定部分
 *  - setPartHidden(hidden, part): 设置部分隐藏状态
 *  - resizePart(part, size): 调整部分大小
 *  - enterZenMode(): 进入禅模式
 *  - exitZenMode(): 退出禅模式
 *
 *  【配置项】
 *  - workbench.editor.showTabs: 显示编辑器标签
 *  - workbench.activityBar.location: 活动栏位置
 *  - zenMode.showTabs: 禅模式显示标签
 *
 *  【使用场景】
 *  - 用户拖动侧边栏调整宽度
 *  - 点击按钮隐藏/显示面板
