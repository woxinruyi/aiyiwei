 ...
 *
 *  【业务逻辑说明 - 工作台配置服务实现】
 *  本文件实现工作台的配置服务，负责管理用户设置和工作区配置：
 *
 *  【核心职责】
 *  1. 管理多层级配置（默认、用户、工作区、文件夹）
 *  2. 处理配置文件读写（settings.json）
 *  3. 监听配置变更并触发事件
 *  4. 支持用户配置档（User Data Profile）
 *  5. 处理策略配置（Policy Configuration）
 *
 *  【配置层级】
 *  ┌─────────────────────────────────────────────────────────┐
 *  │  默认配置（Default）                                     │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  应用配置（Application）- 命令行参数                   │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  用户配置（User）- ~/.vscode/settings.json               │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  远程用户配置（Remote User）- 远程环境用户设置           │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  工作区配置（Workspace）- .vscode/settings.json          │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  文件夹配置（Folder）- 多根工作区文件夹设置               │
 *  └─────────────────────────────────────────────────────────┘
 *
 *  【核心方法】
 *  - getValue(key): 获取配置值
 *  - updateValue(key, value): 更新配置值
 *  - inspect(key): 检查配置来源
 ...
 *  - 应用策略限制（如禁用某些功能）
 *
 *  【与 platform/configurationService.ts 的关系】
 *  - platform 提供基础配置服务
 *  - 本文件扩展以支持工作台特定功能（用户配置档等）
 *
