 ...
 *
 *  【业务逻辑说明 - 编辑器输入基类】
 *  本文件定义编辑器输入（EditorInput）的基类和核心接口，代表编辑器中打开的资源：
 *
 *  【核心职责】
 *  1. 定义编辑器输入抽象基类（AbstractEditorInput）
 *  2. 提供编辑器关闭处理接口（IEditorCloseHandler）
 *  3. 支持编辑器能力和特性标志
 *  4. 管理编辑器输入的生命周期和事件
 *  5. 提供序列化和反序列化支持
 *
 *  【编辑器输入类型】
 *  ┌─────────────────────────────────────────────────────────┐
 *  │  FileEditorInput - 文件编辑器输入                       │
 *  │  - 打开的文件                                            │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  DiffEditorInput - 差异编辑器输入                       │
 *  │  - 对比两个文件                                          │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  ResourceEditorInput - 资源编辑器输入                   │
 *  │  - 无文件关联的编辑器                                     │
 *  ├─────────────────────────────────────────────────────────┤
 *  │  GettingStartedInput - 欢迎页面输入                     │
 *  │  - 欢迎页面编辑器                                        │
 *  └─────────────────────────────────────────────────────────┘
 *
 *  【核心方法】
 ...
 *  - Readonly: 只读
 *  - Untitled: 未命名文件
 *  - RequiresTrust: 需要工作区信任
 *  - Singleton: 单例（只能打开一个）
 *  - Scratchpad: 草稿模式
 *  - CanSplitInGroup: 支持在组内拆分
 *
 *  【使用场景】
 *  - 打开文件时创建编辑器输入
 *  - 编辑器标签页显示
 ...
 *
 *  【与 editor.ts 的关系】
 *  - editor.ts 定义 IEditorPane 和 IEditorInput 接口
 *  - 本文件提供 AbstractEditorInput 基类实现
 *
