 ...
 *
 *  【业务逻辑说明 - 编辑器组模型】
 *  本文件实现编辑器组（Editor Group）的模型管理，负责管理一个编辑器组内的所有编辑器：
 *
 *  【核心职责】
 *  1. 管理组内编辑器的列表（打开、关闭、排序）
 *  2. 维护活动编辑器（Active）和预览编辑器（Preview）
 *  3. 支持固定（Pinned）和粘性（Sticky）编辑器
 *  4. 实现最近使用（MRU）顺序管理
 *  5. 处理编辑器打开位置策略（LEFT/RIGHT/FIRST/LAST）
 *
 *  【编辑器组概念】
 *  ┌─────────────────────────────────────────────────────────┐
 *  │  编辑器组是一个标签页容器，管理多个编辑器               │
 *  │                                                          │
 *  │  ┌─────┬─────┬─────┬─────┬─────────────────────────┐   │
 *  │  │ Tab │ Tab │ Tab │ Tab │        (更多...)        │   │
 *  │  │  1  │  2  │  3* │  4  │                         │   │
 ...
 *  │  特性：                                                  │
 *  │  - Pinned: 固定标签（不会自动关闭）                     │
 *  │  - Sticky: 粘性标签（始终显示在最左侧）                 │
 *  │  - Preview: 预览模式（单击文件时临时打开）              │
 *  │  - MRU: 最近使用顺序（Ctrl+Tab 切换）                  │
 *  └─────────────────────────────────────────────────────────┘
 *
 *  【打开位置策略】
 *  - LEFT: 在活动编辑器左侧打开
 ...
 *
 *  【核心接口】
 *  - IEditorOpenOptions: 打开编辑器的选项
 *  - ISerializedEditorGroupModel: 序列化的组状态
 *  - EditorGroupModel: 编辑器组模型类
 *
 *  【事件系统】
 *  - onDidModelChange: 模型变更事件（打开、关闭、激活等）
 *  - onDidActiveEditorChange: 活动编辑器变更
 *  - onDidMoveEditor: 编辑器移动事件
 *
 *  【使用场景】
 *  - 管理编辑器组的标签页
 *  - 处理编辑器打开/关闭/切换
 ...
 *
 *  【与 editorPart.ts 的关系】
 *  - editorPart.ts 管理多个编辑器组
 *  - editorGroupModel.ts 管理单个组内的编辑器
 *
