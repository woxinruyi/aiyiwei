 ...
 *
 *  【业务逻辑说明 - 并排编辑器输入】
 *  本文件实现并排编辑器（Side-by-Side Editor）的输入类，用于同时显示两个编辑器：
 *
 *  【核心职责】
 *  1. 管理主编辑器（primary）和辅助编辑器（secondary）
 *  2. 协调两个编辑器输入的生命周期和事件
 *  3. 处理保存和恢复操作（双向同步）
 *  4. 支持编辑器组的移动和复制
 *  5. 管理并排编辑器的复合能力（capabilities）
 *
 *  【并排编辑器概念】
 *  ┌─────────────────────────────────────────────────────────┐
 *  │  SideBySideEditorInput 包含两个编辑器输入             │
 *  │                                                          │
 *  │  ┌──────────────┬──────────────┐                       │
 *  │  │  Primary     │  Secondary   │                       │
 *  │  │  主编辑器    │  辅助编辑器   │                       │
 ...
 *  【复合能力管理】
 *  - 主编辑器的能力作为基础能力
 *  - 合并两个编辑器的特殊能力（RequiresTrust, Singleton）
 *  - 标记为 MultipleEditors 能力
 *  - 移除 CanSplitInGroup 能力（不适用于并排编辑器）
 *
 *  【核心方法】
 *  - getPrimary(): 获取主编辑器输入
 *  - getSecondary(): 获取辅助编辑器输入
 *  - save(): 保存两个编辑器
 *  - revert(): 恢复两个编辑器
 *  - moveTo(): 移动到目标组
 *  - serialize(): 序列化状态
 *
 *  【事件处理】
 *  - 转发主编辑器的事件（onDidChangeDirty, onDidChangeLabel）
 *  - 监控两个编辑器的变更
 *  - 脏状态变更时触发整体更新
 *
 *  【继承关系】
 ...
 *  │  EditorInput（编辑器输入基类）                          │
 *  │       ↑ 继承                                            │
 *  │  SideBySideEditorInput（本类）                         │
 *  │       ↑ 继承                                            │
 *  │  DiffEditorInput（差异编辑器输入）                      │
 *  └─────────────────────────────────────────────────────────┘
 *
 *  【使用场景】
 *  - 差异编辑器的基础实现
