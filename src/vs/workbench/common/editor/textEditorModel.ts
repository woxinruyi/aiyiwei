 ...
 *
 *  【业务逻辑说明 - 文本编辑器模型基类】
 *  本文件实现文本编辑器模型的基础类，继承自 EditorModel 并集成代码编辑器模型：
 *
 *  【核心职责】
 *  1. 集成代码编辑器的文本模型（ITextModel）
 *  2. 管理语言支持和语言检测
 *  3. 处理模型创建和销毁生命周期
 *  4. 支持语言自动检测（延迟 600ms）
 *  5. 提供无障碍访问支持
 ...
 *  │  EditorModel（编辑器模型基类）                          │
 *  │       ↑ 继承                                            │
 *  │  BaseTextEditorModel（本基类）                         │
 *  │       ↑ 继承                                            │
 *  │  TextFileEditorModel（文本文件模型）                   │
 *  │       ↑ 继承                                            │
 *  │  UntitledTextEditorModel（未命名文件模型）             │
 *  └─────────────────────────────────────────────────────────┘
 *
 *  【核心功能】
 *  ┌─────────────────────────────────────────────────────────┐
 ...
 *
 *  【核心方法】
 *  - createTextEditorModel(): 创建文本编辑器模型
 *  - destroyTextEditorModel(): 销毁模型
 *  - handleExistingModel(): 处理已有模型
 *  - autoDetectLanguage(): 自动检测语言
 *  - getLanguageSelection(): 获取语言选择
 *
 *  【使用场景】
 *  - 打开文本文件时创建模型
 *  - 新建未命名文件时创建模型
 ...
 *  - 关闭编辑器时销毁模型
 *
 *  【与 editorModel.ts 的关系】
 *  - 继承 EditorModel 的基础生命周期
 *  - 添加文本模型特有的功能（语言、内容）
 *
