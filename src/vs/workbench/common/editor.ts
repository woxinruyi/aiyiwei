 ...
 *
 *  【业务逻辑说明 - 编辑器核心接口】
 *  本文件定义编辑器的核心接口和类型，是工作台编辑器系统的基础：
 *
 *  【核心职责】
 *  1. 定义编辑器面板接口（IEditorPane）
 *  2. 定义编辑器输入接口（IUntypedEditorInput）
 *  3. 提供编辑器扩展点（EditorExtensions）
 *  4. 定义编辑器关联和类型
 *  5. 支持编辑器选项和状态管理
 *
 *  【编辑器类型】
 ...
 *  【核心接口】
 *  - IEditorPane: 编辑器面板接口
 *  - IEditorInput: 编辑器输入抽象
 *  - IUntypedEditorInput: 无类型编辑器输入
 *  - IEditorOptions: 编辑器选项
 *  - IEditorGroup: 编辑器组接口
 *
 *  【编辑器扩展点】
 *  - EditorExtensions.EditorPane: 编辑器面板注册
 *  - EditorExtensions.EditorFactory: 编辑器输入工厂注册
 *
 *  【使用场景】
 *  - 打开文件时创建编辑器
 *  - 编辑器切换和分组
